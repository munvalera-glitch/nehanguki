...
def insert_signature(page, b64_data, rect_coords):
    """
    Inserts a base64 encoded PNG signature into the given PyMuPDF page.
...
def generate_application_form(data: dict, template_path: str, output_path: str):
    ...
    if sex in ("M", "MALE"):
        page.insert_text(fitz.Point(364.5, 329), "v", fontsize=14, fontname="nanum", fontfile=FONT, color=(0, 0, 0))
    elif sex in ("F", "FEMALE"):
        page.insert_text(fitz.Point(364.5, 340), "v", fontsize=14, fontname="nanum", fontfile=FONT, color=(0, 0, 0))

    # ── Action checkboxes ────────────────────────────────────────────────────
    action = data.get("action", "").lower().strip()
    if action == "initial":
        page.insert_text(fitz.Point(64.0, 148.5), "v", fontsize=14, fontname="nanum", fontfile=FONT, color=(0, 0, 0))
    elif action == "reissue":
        page.insert_text(fitz.Point(63.0, 182.8), "v", fontsize=14, fontname="nanum", fontfile=FONT, color=(0, 0, 0))
    elif action == "extension":
        page.insert_text(fitz.Point(63.0, 205.8), "v", fontsize=14, fontname="nanum", fontfile=FONT, color=(0, 0, 0))
    elif action == "address_change":
        page.insert_text(fitz.Point(222.0, 228.8), "v", fontsize=14, fontname="nanum", fontfile=FONT, color=(0, 0, 0))
    ...
    is_f4 = data.get("visaType") == "F4"
    
    # ── Date ──
    import datetime
    app_date = data.get("application_date", "")
    if app_date and len(app_date.split("-")) == 3:
        y, m, d = app_date.split("-")
    else:
    ...
    if full_name_sig:
        page.insert_text(fitz.Point(438, 586), full_name_sig, fontsize=6, fontname="nanum", fontfile=FONT, color=(0,0,0))
        page.insert_text(fitz.Point(97, 700), full_name_sig, fontsize=6, fontname="nanum", fontfile=FONT, color=(0,0,0))
    page.insert_text(fitz.Point(162, 585), date_str, fontsize=8, fontname="nanum", fontfile=FONT, color=(0,0,0))

    if sigs.get("applicant"):
        insert_signature(page, sigs["applicant"], (491.7, 567.8, 491.7 + 81.54, 567.8 + 36.92))
        insert_signature(page, sigs["applicant"], (138.7, 687.8, 138.7 + 81.54, 687.8 + 36.92))
...
def generate_accommodation_form(data: dict, template_path: str, output_path: str):
    ...
    # ── CHECKBOXES (TOP-LEFT) ──
    rel_coords = {
        "family_relative": (180.42, 336.99),
        "employer": (270.52, 336.99),
        "other": (366.19, 337.05)
    }
    rel = data.get("acc_relationship", "")
    if rel in rel_coords:
        page.insert_text(fitz.Point(rel_coords[rel][0] - 1.5, rel_coords[rel][1] + 2), "V", fontsize=15, fontname="nanum", fontfile=FONT, color=(0,0,0))
    ...
    own = data.get("acc_ownership_type", "")
    if own in own_coords:
        page.insert_text(fitz.Point(own_coords[own][0] - 1.5, own_coords[own][1] + 2), "V", fontsize=15, fontname="nanum", fontfile=FONT, color=(0,0,0))

    res_coords = {
        "private_residence": (180.42, 399.45),
        "dormitory": (324.58, 399.45),
        "accommodation": (181.01, 428.59),
        "other": (325.23, 428.59)
    }
    res = data.get("acc_residence_type", "")
    if res in res_coords:
        page.insert_text(fitz.Point(res_coords[res][0] - 1.5, res_coords[res][1] + 2), "V", fontsize=15, fontname="nanum", fontfile=FONT, color=(0,0,0))
    ...
    # Starting Date
    page.insert_text(fitz.Point(187.69, 460.80 - 1), yyyy, fontsize=10, fontname="nanum", fontfile=FONT, color=(0,0,0))
    page.insert_text(fitz.Point(266.92, 461.55 - 1), mm, fontsize=10, fontname="nanum", fontfile=FONT, color=(0,0,0))
    page.insert_text(fitz.Point(358.46, 459.25 - 1), dd, fontsize=10, fontname="nanum", fontfile=FONT, color=(0,0,0))

    # Confirmation Date
    page.insert_text(fitz.Point(180.77, 524.63 - 1), yyyy, fontsize=10, fontname="nanum", fontfile=FONT, color=(0,0,0))
    page.insert_text(fitz.Point(258.46, 524.63 - 1), mm, fontsize=10, fontname="nanum", fontfile=FONT, color=(0,0,0))
    page.insert_text(fitz.Point(323.85, 523.10 - 1), dd, fontsize=10, fontname="nanum", fontfile=FONT, color=(0,0,0))

    # ── SIGNATURES (TOP-LEFT) ──
    sigs = data.get("signatures", {})
    
    # Provider Signature
    if sigs.get("accommodationProvider"):
        insert_signature(page, sigs["accommodationProvider"], (312.64, 521.63, 312.64 + 81.54, 521.63 + 36.92))
        
    if data.get("provider_full_name"):
        page.insert_text(
            fitz.Point(263, 544),
...
def generate_occupation_form(data: dict, template_path: str, output_path: str):
    ...
    if sex_occ in ("M", "MALE"):
        page.insert_text((351.5, 287), "v", fontsize=14, fontname="nanum", fontfile=FONT, color=(0, 0, 0))
    elif sex_occ in ("F", "FEMALE"):
        page.insert_text((349.5, 298), "v", fontsize=14, fontname="nanum", fontfile=FONT, color=(0, 0, 0))
    ...
    # === ГАЛОЧКА ПРОФЕССИИ (occupationType) ===
    # Ключи — значения поля occupationType из фронтенда.
    # Значение: (page_index, x, y) — координаты для вставки "v" внутри □
    OCCUPATION_COORDS = {
        # ── Ключи фронтенда (упрощённые) ───────────────────────────────────
        "unemployed":    (0, 62, 363.7),   # □무직 No occupation
        "production":    (2, 60, 166),   # □기계 (Machine Production)
        "warehouse":     (2, 60, 213),   # □운전 (Transport/Driving)
        "construction":  (2, 60, 305),   # □건설 Elementary construction
        "retail":        (1, 60, 360),   # □매장 Store sales
        "office":        (0, 60, 437),   # □행정∙경영 Admin/Management
        "business":      (0, 60, 411),   # □공공 기관 / senior official
        "manual":        None,           # "вручную" — без галочки

        # ── Детальные ключи (на будущее) ────────────────────────────────────
        "manager_public":           (0, 60, 411),
        "manager_admin":            (0, 60, 437),
        "manager_professional":     (0, 60, 463),
        "manager_construction":     (0, 60, 489),
        "manager_sales":            (0, 60, 515),
        "professional_science":     (0, 60, 566),
        "professional_ict":         (0, 60, 595),
        "professional_engineering": (0, 60, 625),
        "professional_health":      (0, 60, 654),
        "professional_education":   (0, 60, 684),
        "professional_legal":       (0, 60, 713),
        "professional_finance":     (0, 60, 743),
        "professional_arts":        (0, 60, 772),
        "clerk_admin":              (1, 60, 75),
        "clerk_finance":            (1, 60, 101),
        "clerk_legal":              (1, 60, 127),
        "clerk_customer":           (1, 60, 152),
        "service_police":           (1, 60, 208),
        "service_care":             (1, 60, 234),
        "service_transport":        (1, 60, 260),
        "service_food":             (1, 60, 286),
        "sales_direct":             (1, 60, 334),
        "sales_store":              (1, 60, 360),
        "sales_mobile":             (1, 60, 386),
        "agriculture":              (1, 60, 435),
        "forestry":                 (1, 60, 461),
        "fishery":                  (1, 60, 487),
        "craft_food":               (1, 60, 538),
        "craft_textile":            (1, 60, 568),
        "craft_wood":               (1, 60, 593),
        "craft_metal":              (1, 60, 627),
        "craft_transport":          (1, 60, 657),
        "craft_electric":           (1, 60, 687),
        "craft_ict":                (1, 60, 717),
        "craft_construction":       (1, 60, 746),
        "craft_other":              (1, 60, 776),
        "machine_food":             (2, 60, 74),
        "machine_textile":          (2, 60, 97),
        "machine_chemical":         (2, 60, 120),
        "machine_metal":            (2, 60, 143),
        "machine_production":       (2, 60, 166),
        "machine_electric":         (2, 60, 190),
        "machine_driving":          (2, 60, 213),
        "machine_water":            (2, 60, 236),
        "machine_print":            (2, 60, 259),
        "elementary_construction":  (2, 60, 305),
        "elementary_transport":     (2, 60, 328),
        "elementary_production":    (2, 60, 352),
        "elementary_cleaning":      (2, 60, 375),
        "elementary_household":     (2, 60, 398),
        "elementary_agriculture":   (2, 60, 430),
    }

    occ_type = (data.get("occupationType") or "").strip().lower()
    coords = OCCUPATION_COORDS.get(occ_type)
    if coords is not None:  # None = manual, skip
        pg_i, chk_x, chk_y = coords
        if pg_i < len(doc):
            doc[pg_i].insert_text(
                fitz.Point(chk_x - 4.5, chk_y + 8),  # Shifted 1mm higher (-3pt) and 1mm left (-3pt) relative to chk_x - 1.5, chk_y + 11
                "v",
                fontsize=14,
                fontname="nanum",
                fontfile=FONT,
    ...
    if len(doc) > 2:
        ...
        now = datetime.datetime.now()
        y, m, d = str(now.year), f"{now.month:02d}", f"{now.day:02d}"
        page3.insert_text(fitz.Point(363.30, 523.00), y, fontsize=9, fontname="nanum", fontfile=FONT, color=(0,0,0))
        page3.insert_text(fitz.Point(413.30, 523.00), m, fontsize=9, fontname="nanum", fontfile=FONT, color=(0,0,0))
        page3.insert_text(fitz.Point(458.30, 523.00), d, fontsize=9, fontname="nanum", fontfile=FONT, color=(0,0,0))
        
        # ── Applicant Name ──
        if full_name:
            page3.insert_text(fitz.Point(330.00, 555.00), full_name, fontsize=9, fontname="nanum", fontfile=FONT, color=(0,0,0))

        sigs = data.get("signatures", {})
        if sigs.get("applicant"):
            insert_signature(page3, sigs["applicant"], (422, 544.3, 422 + 81.54, 544.3 + 36.92))
...
def generate_guarantee_form(app_data: dict, guarantor_data: dict, template_path: str, output_path: str):
    ...
    if sex_app in ("M", "MALE"):
        page.insert_text(fitz.Point(466.5, 162), "v", fontsize=14, fontname="nanum", fontfile=FONT, color=(0, 0, 0))
    elif sex_app in ("F", "FEMALE"):
        page.insert_text(fitz.Point(467.5, 176), "v", fontsize=14, fontname="nanum", fontfile=FONT, color=(0, 0, 0))
    ...
    if sex_guar in ("M", "MALE"):
        page.insert_text(fitz.Point(465.5, 356), "v", fontsize=14, fontname="nanum", fontfile=FONT, color=(0, 0, 0))
    elif sex_guar in ("F", "FEMALE"):
        page.insert_text(fitz.Point(466.5, 368), "v", fontsize=14, fontname="nanum", fontfile=FONT, color=(0, 0, 0))
    ...
    now = datetime.datetime.now()
    y, m, d = str(now.year), f"{now.month:02d}", f"{now.day:02d}"
    page.insert_text(fitz.Point(354, 744), y, fontsize=9, fontname="nanum", fontfile=FONT, color=(0,0,0))
    page.insert_text(fitz.Point(418, 744), m, fontsize=9, fontname="nanum", fontfile=FONT, color=(0,0,0))
    page.insert_text(fitz.Point(478, 744), d, fontsize=9, fontname="nanum", fontfile=FONT, color=(0,0,0))

    # ── ПОДПИСЬ ГАРАНТА (имя) ──────────────────────────────
    # Guarantor name unconditionally at the requested coordinate
    guar = guarantor_data.get("guarantor_full_name", "")
    if guar:
        page.insert_text(fitz.Point(344, 765), guar, fontsize=9, fontname="nanum", fontfile=FONT, color=(0,0,0))

    sigs = guarantor_data.get("signatures", {})
    if sigs.get("guarantor"):
        insert_signature(page, sigs["guarantor"], (449, 745.3, 449 + 81.54, 745.3 + 36.92))
...
def generate_goso_f4_form(data: dict, template_path: str, output_path: str):
    ...
    sex = data.get("sex", "").upper().strip()
    if sex in ("M", "MALE"):
        page.insert_text(fitz.Point(403.0, 287.5), "v", fontsize=14, fontname="nanum", fontfile=FONT, color=(0, 0, 0))
    elif sex in ("F", "FEMALE"):
        page.insert_text(fitz.Point(404.0, 298.5), "v", fontsize=14, fontname="nanum", fontfile=FONT, color=(0, 0, 0))

    # ── Action checkboxes ────────────────────────────────────────────────────
    action = data.get("action", "").lower().strip()
    if action == "initial":
        page.insert_text(fitz.Point(63.5, 129.0), "v", fontsize=14, fontname="nanum", fontfile=FONT, color=(0, 0, 0))
    elif action == "extension":
        page.insert_text(fitz.Point(268.5, 129.0), "v", fontsize=14, fontname="nanum", fontfile=FONT, color=(0, 0, 0))
    elif action == "reissue":
        page.insert_text(fitz.Point(63.5, 215.0), "v", fontsize=14, fontname="nanum", fontfile=FONT, color=(0, 0, 0))
    elif action == "address_change":
        page.insert_text(fitz.Point(63.5, 169.5), "v", fontsize=14, fontname="nanum", fontfile=FONT, color=(0, 0, 0))
    ...
    date_str = f"{y}          {m}          {d}"
    page.insert_text(fitz.Point(228, 561), date_str, fontsize=8, fontname="nanum", fontfile=FONT, color=(0,0,0))

    # ── 신청인 서명 Applicant Signature ───────────────────────────────────────
    full = f"{data.get('surname','')} {data.get('given_names','')}".strip()
    if full:
        page.insert_text(fitz.Point(455, 561), full, fontsize=6, fontname="nanum", fontfile=FONT, color=(0,0,0))
        page.insert_text(fitz.Point(99, 666), full, fontsize=6, fontname="nanum", fontfile=FONT, color=(0,0,0))

    sigs = data.get("signatures", {})
    if sigs.get("applicant"):
        insert_signature(page, sigs["applicant"], (484, 548.3, 484 + 81.54, 548.3 + 36.92))
        insert_signature(page, sigs["applicant"], (149, 664.3, 149 + 81.54, 664.3 + 36.92))
...
def generate_school_form(data: dict, template_path: str, output_path: str):
    ...
    page.insert_text(fitz.Point(394, 659), y, fontsize=9, fontname="nanum", fontfile=FONT, color=(0,0,0))
    page.insert_text(fitz.Point(441, 658), m, fontsize=9, fontname="nanum", fontfile=FONT, color=(0,0,0))
    page.insert_text(fitz.Point(481, 658), d, fontsize=9, fontname="nanum", fontfile=FONT, color=(0,0,0))

    # ── 신청인 Applicant (signature line) ────────────────────────────────────
    # Print the applicant's name unconditionally at the requested coordinates
    full = data.get("full_name", "")
    if full:
        page.insert_text(fitz.Point(325, 707), full, fontsize=8, fontname="nanum", fontfile=FONT, color=(0,0,0))

    sigs = data.get("signatures", {})
    if sigs.get("applicant"):
        insert_signature(page, sigs["applicant"], (430, 698, 430 + 81.54, 698 + 36.92))
...
def generate_scan_pages(files_dict, output_path):
    ...
    idcard_back_path = files_dict.get("idCardBack")
    contract_path = files_dict.get("contract")
    provider_idcard_path = files_dict.get("providerIdCard")
    guarantor_passport_path = files_dict.get("guarantorPassport")
    ...
    def insert_compressed_image(page, rect, image_path):
        ...
        try:
            ...
            with Image.open(image_path) as img:
                ...
                buffer.seek(0)
                page.insert_image(rect, stream=buffer.read(), keep_proportion=True)
        except Exception as e:
            print(f"Image compression failed for {image_path}: {e}")
            page.insert_image(rect, filename=image_path, keep_proportion=True)
    
    # If we have both front and back ID card but no passport (like in password recovery)
    if idcard_path and idcard_back_path and not passport_path:
        page = doc.new_page(width=A4_WIDTH, height=A4_HEIGHT)
        
        ix0 = (A4_WIDTH - ID_W) / 2
        iy0 = (A4_HEIGHT / 2 - ID_H) / 2
        rect = fitz.Rect(ix0, iy0, ix0 + ID_W, iy0 + ID_H)
        insert_compressed_image(page, rect, idcard_path)
            
        ix1 = (A4_WIDTH - ID_W) / 2
        iy1 = A4_HEIGHT / 2 + (A4_HEIGHT / 2 - ID_H) / 2
        rect = fitz.Rect(ix1, iy1, ix1 + ID_W, iy1 + ID_H)
        insert_compressed_image(page, rect, idcard_back_path)
        ...
        if has_page_1:
            ...
            if passport_path and os.path.exists(passport_path):
                ...
                insert_compressed_image(page, rect, passport_path)
        ...
        if contract_path and os.path.exists(contract_path):
            ...
            insert_compressed_image(page, rect, contract_path)
            
        if guarantor_passport_path and os.path.exists(guarantor_passport_path):
            page = doc.new_page(width=A4_WIDTH, height=A4_HEIGHT)
            px0 = (A4_WIDTH - PASS_W) / 2
            py0 = (A4_HEIGHT / 2 - PASS_H) / 2
            rect = fitz.Rect(px0, py0, px0 + PASS_W, py0 + PASS_H)
            insert_compressed_image(page, rect, guarantor_passport_path)
            
        if provider_idcard_path and os.path.exists(provider_idcard_path):
            page = doc.new_page(width=A4_WIDTH, height=A4_HEIGHT)
            ix0 = (A4_WIDTH - ID_W) / 2
            iy0_upper = (A4_HEIGHT / 2 - ID_H) / 2
            rect_upper = fitz.Rect(ix0, iy0_upper, ix0 + ID_W, iy0_upper + ID_H)
            insert_compressed_image(page, rect_upper, provider_idcard_path)
        
    if len(doc) > 0:
        doc.save(output_path, deflate=True, garbage=4)
    doc.close()

def generate_password_recovery_form(data: dict, signature_path: str, template_path: str, output_path: str, idcard_path: str = "", idcard_back_path: str = ""):
    """
    Заполняет форму Восстановление пароля на HiKorea.
    """
    doc = fitz.open(template_path)
    page = doc[0]
    ...
    if app_date and len(app_date.split("-")) == 3:
        y, m, d = app_date.split("-")
        y = y[-2:] # Get only last 2 digits
        formatted_date = f"{y}년 {m}월 {d}일"
        # Using exact placement (147, 97) instead of textbox to prevent overflow hiding
        t(147, 97, formatted_date, size=10, korean=True)

    # 2. HiKorea ID
    # 1 mm up (-2.8 pt) -> 157.2 - 2.8 = 154.4
    t(203.8, 154.4, data.get("hikorea_id", ""), size=9)

    # 3. Контактный телефон
    t(429.2, 154.1, data.get("cell_phone", ""), size=9)

    # 4. Имя / Name
    full_name = data.get("fullName") or f"{data.get('surname', '')} {data.get('given_names', '')}".strip()
    t(202.8, 181.0, full_name, size=9, korean=True)
    ...
    if signature_path and os.path.exists(signature_path):
        ...
        try:
            ...
            new_data = []
            for item in datas:
                # If pixel is bright or already transparent -> transparent
                if item[3] < 128 or (item[0] > 200 and item[1] > 200 and item[2] > 200):
                    new_data.append((255, 255, 255, 0))
                else:
                    new_data.append((0, 0, 0, 255)) # Make stroke black
            sig_img.putdata(new_data)
            trans_sig_path = signature_path + "_trans.png"
            sig_img.save(trans_sig_path, "PNG")
            
            rect = fitz.Rect(393, 160, 393 + 82, 160 + 41)
            page.insert_image(rect, filename=trans_sig_path, keep_proportion=True)
        except Exception as e:
            # Fallback
            rect = fitz.Rect(393, 160, 393 + 82, 160 + 41)
    ...
    t(200.7, 210.3, data.get("arc", ""), size=9)

    # 7. Дата рождения
    dob = f"{data.get('birth_year', '')}.{data.get('birth_month', '')}.{data.get('birth_day', '')}"
    t(428.9, 210.3, dob, size=9)

    # 8. Текущий адрес
    t(203.0, 264.1, data.get("address", ""), size=8, korean=True)

    # 9. Сообщение вебмастеру
    t(203.5, 291.8, data.get("webmaster_message", ""), size=8, korean=True)

    # 10 & 11. Копии ID (Лицевая и оборотная стороны)
    def process_id_card(path):
        from PIL import Image
        try:
            with Image.open(path) as im:
                ...
                if h > w:
                    ...
                    new_path = path + "_horiz.jpg"
                    rot.convert('RGB').save(new_path, "JPEG")
                    return new_path
        except:
            pass
        return path
    ...
    if idcard_path and os.path.exists(idcard_path):
        processed_path = process_id_card(idcard_path)
        rect_front = fitz.Rect(72.3, 385.7, 72.3 + ID_W, 385.7 + ID_H)
        page.insert_image(rect_front, filename=processed_path, keep_proportion=True)

    if idcard_back_path and os.path.exists(idcard_back_path):
        processed_path = process_id_card(idcard_back_path)
        # Place back ID horizontally as requested (not rotated 270 degrees in PyMuPDF)
        # Using the same X coordinate as before, but height and width are horizontal now
        rect_back = fitz.Rect(68.9, 580.7, 68.9 + ID_W, 580.7 + ID_H)
        page.insert_image(rect_back, filename=processed_path, keep_proportion=True)

    doc.select([0])  # Убираем вторую страницу, оставляем только первую
    doc.save(output_path)
    doc.close()
