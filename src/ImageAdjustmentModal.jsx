...
export default function ImageAdjustmentModal({ imageSrc, onSave, onCancel, forceLanguage }) {
    ...
    return (
        <div className="fixed top-0 left-0 w-full h-[100dvh] z-[9999] flex flex-col bg-[#111]" style={{ touchAction: "none" }}>
            {/* Top bar */}
            <div className="flex items-center justify-between px-3 py-3 border-b border-white/10" style={{ background: "rgba(0,0,0,0.92)" }}>
                ...
                <div className="text-[14px] font-semibold text-white">{lang === 'ko' ? '이미지 조정' : 'Настройка изображения'}</div>
                ...
                >
                    {saving ? (
                        ...
                        <>
                            ...
                            {lang === 'ko' ? '완료 - 이미지 사용' : 'Готово - Использовать'}
                        </>
                    )}
                </button>
            </div>
