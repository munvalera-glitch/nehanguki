 ...
 */
export default function ImageEditorModal({ file, docType, aspectRatio, helperText, onSave, onCancel, forceLanguage }) {
    ...
    }, [completedCrop, cornerPoints, docType, onSave, t, lang]);
        ...
        >
            ...
            <div className="flex items-center justify-center gap-2 px-4 py-1.5 bg-white/5 border-b border-white/5">
                <span className="text-[11px] text-white/40">
                    {docType 
                        ? (lang === 'ko' ? '모서리를 이동시켜 문서에 맞추세요' : 'Переместите углы, чтобы они совпадали с документом')
                        : `${lang === 'ko' ? '드래그하여 자르기' : 'Перетащите углы'} • ${lang === 'ko' ? '프레임 이동' : 'Переместите рамку'} • ${lang === 'ko' ? '핀치하여 확대' : 'Используйте зум'}`}
                </span>
            </div>
                ...
                >
                    {saving ? (
                        <>
                            ...
                            {lang === 'ko' ? '저장 중...' : 'Сохранение...'}
                        </>
                    ) : (
                        <>
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
                            {docType ? (lang === 'ko' ? '완료' : 'Готово') : (lang === 'ko' ? '선택 영역 사용' : 'Готово - Использовать')}
                        </>
                    )}
                </button>
            </div>
