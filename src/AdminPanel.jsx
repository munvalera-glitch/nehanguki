...
function StatusBadge({ user }) {
    ...
    if (!user.freeGenerationUsed) return <span className="text-xs px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 font-bold">1 бесплатно</span>;
    return <span className="text-xs px-2 py-0.5 rounded-full bg-gray-100 text-gray-500 font-bold">Обычный</span>;
}

function EditModal({ user, onClose, onSave, t }) {
    const currentPerm = user.permanentCredits ?? user.paidGenerationsRemaining ?? 0;
    const currentTmp = user.temporaryCredits || 0;
    const lifetimeSpent = user.lifetimeCreditsSpent || 0;

    const [addPermanent, setAddPermanent] = useState("");
    const [addTemporary, setAddTemporary] = useState("");
    ...
    const [isB2B, setIsB2B] = useState(!!user.isB2B);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (addPermanent !== "") payload.addPermanentCredits = Number(addPermanent);
            if (addTemporary !== "") payload.addTemporaryCredits = Number(addTemporary);
    ...
    return (
        <div className="fixed inset-0 z-[300] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-4 animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
                ...
                <div className="bg-[#f7f7f5] rounded-xl p-3 flex justify-between items-center text-sm">
                    <span className="text-[#5f6368]">Всего потрачено кредитов:</span>
                    <span className="font-bold text-[#1a1c1d]">{lifetimeSpent}</span>
                </div>

                {/* Section 1: Permanent credits */}
                <Section label="🔑 Постоянные кредиты" color="bg-green-50 border-green-200">
                    <div className="flex items-center justify-between text-sm text-[#5f6368] mb-1">
                        <span>Сейчас: <strong className="text-green-700">{currentPerm}</strong></span>
                        {addPermanent !== "" && <span>Будет: <strong className="text-[#1a1c1d]">{Math.max(0, currentPerm + Number(addPermanent))}</strong></span>}
                    </div>
                    <div className="flex items-center gap-3">
                        <input
                            type="number" placeholder="Добавить / отнять" value={addPermanent}
                            onChange={e => setAddPermanent(e.target.value)}
                            className="flex-1 px-3 py-2 border border-[#e7e5e2] rounded-xl text-[15px] font-bold focus:border-green-400 focus:ring-1 focus:ring-green-400 outline-none"
                        />
                    </div>
                    <p className="text-xs text-[#999]">Напишите число для пополнения (например 15) или списания (-5)</p>
                </Section>

                {/* Section 2: Temporary credits */}
                <Section label="⏱ Временные кредиты (истекают)" color="bg-blue-50 border-blue-200">
                    <div className="flex items-center justify-between text-sm text-[#5f6368] mb-1">
                        <span>Сейчас: <strong className="text-blue-700">{currentTmp}</strong></span>
                        {addTemporary !== "" && <span>Будет: <strong className="text-[#1a1c1d]">{Math.max(0, currentTmp + Number(addTemporary))}</strong></span>}
                    </div>
                    <div className="flex items-center gap-3">
                        <input
                            type="number" placeholder="Добавить / отнять" value={addTemporary}
                            onChange={e => setAddTemporary(e.target.value)}
                            className="flex-1 px-3 py-2 border border-[#e7e5e2] rounded-xl text-[15px] font-bold focus:border-blue-400 focus:ring-1 focus:ring-blue-400 outline-none"
                        />
                    </div>
                    <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            />
                            {tmpExpiry && <button onClick={() => { setAddTemporary(-currentTmp); setTmpExpiry(""); }} className="text-xs text-red-400 hover:text-red-600 underline">Сбросить</button>}
                        </div>
                    </div>
                </Section>
                ...
                {/* Section 4: B2B */}
                <Section label="🏢 B2B Клиент" color="bg-orange-50 border-orange-200">
                    <label className="flex items-center gap-3 cursor-pointer">
                        <input
                            type="checkbox"
                            checked={isB2B}
                            onChange={e => setIsB2B(e.target.checked)}
                            className="w-5 h-5 rounded border-[#e7e5e2] text-[#2f3437] focus:ring-[#2f3437]"
                        />
                        <span className="font-bold text-[#1a1c1d]">
                            {t ? (t('admin.b2b') || 'B2B клиент') : 'B2B клиент'}
                        </span>
                    </label>
                    <p className="text-xs text-[#999]">Пользователь получит доступ к блоку оплаты на странице "Моя страница"</p>
                </Section>
