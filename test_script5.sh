...
sed -i '/app.use(passport.session());/a \
app.use((req, res, next) => { req.user = { id: "85d91fce-39ed-4bcb-aeb2-2e7d97480f11", email: "munvalera@gmail.com" }; req.isAuthenticated = () => true; next(); }); \
' server_test.js
...
PKG_ID=$(cat res.json | grep -o '"packageId":"[^"]*"' | head -n 1 | cut -d'"' -f4)
...
PKG_ID2=$(cat res2.json | grep -o '"packageId":"[^"]*"' | head -n 1 | cut -d'"' -f4)
...
rm server_test.js dummy.jpg res.json res2.json test_std.pdf test_pwd.pdf test_script5.sh
echo "Done."
